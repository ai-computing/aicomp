...
#
#   This code is for fine-tuning the Hugging Face LLaMA model using 2D parallelism (TP+DP) in vanilla PyTorch.
#
...
if access_token is None:
    raise ValueError("LLAMA_ACCESS_TOKEN environment variable is not set."
                     "       [Usage:] torchrun --nproc_per_node=<#_of_GPUs_per_node> --nnodes=<#_of_nodes> --node_rank=<current_node_rank> --master_addr=<IP_of_rank_0> --master_port=29500 tp_test.py <llama_access_token>")
elif rank == 0:
...
#optimus_t = Optimus_t(model, dp_size=4, tp_size=1)
#optimus_t = Optimus_t(model, dp_size=1, tp_size=8)
#optimus_t = Optimus_t(model, dp_size=4, tp_size=2)
#optimus_t = Optimus_t(model, dp_size=2, tp_size=4)
#optimus_t = Optimus_t(model, dp_size=8, tp_size=1)
optimus_t = Optimus_t(model, dp_size=2, tp_size=8)
#optimus_t = Optimus_t(model, dp_size=3, tp_size=2)
