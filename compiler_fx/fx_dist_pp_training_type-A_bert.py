...
#
#   Micro-batch is supported in this PoC, and applied to the BERT model (CPU version)
#
#
#  Sample Usage:
#      <machine #0>
#            torchrun --nproc_per_node=1 --nnodes=2 --node_rank=0
#                  --master_addr="X.X.X.X" --master_port=29500 fx_dist_pp_training_type-A_bert.py
#      <machine #1>
...
class LossWrapper(BertPreTrainedModel):
...
class Simple_split_test(object):
    ...
    def metadata_transfer(self):
        ...
        if use_wrapper == True:
            config = BertConfig(use_cache=False)
...
#config = BertConfig(vocab_size=50257, embed_dim=4096, use_cache=False)
#config = BertConfig(use_cache=False)
config = BertConfig(is_decoder=True)

#model = BertLMHeadModel.from_pretrained("bert-base-cased", config=config, ignore_mismatched_sizes=True)
