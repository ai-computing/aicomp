...
### Generate shuffling strategy
mkdir -p ${DATADIR}/hdf5/training-${SHARDS}/hdf5_${SHARDS}_shards_varlength
python3 ${SCRIPT_DIR}/shuffle_samples.py \
  --input_hdf5 ${DATADIR}/hdf5/training-${SHARDS}/hdf5_${SHARDS}_shards_uncompressed \
  --output_hdf5 ${DATADIR}/hdf5/training-${SHARDS}/hdf5_${SHARDS}_shards_varlength

fi

### Shuffle and write shards according to dumped strategy in step above
python3 ${SCRIPT_DIR}/shuffle_samples_write.py \
  --input_hdf5 ${DATADIR}/hdf5/training-${SHARDS}/hdf5_${SHARDS}_shards_uncompressed \
  --output_hdf5 ${DATADIR}/hdf5/training-${SHARDS}/hdf5_${SHARDS}_shards_varlength
