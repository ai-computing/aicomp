...
class Comm:
    ...
    def init_comm(self, use_gpu):
        ...
        if use_gpu == True:
            ...
            if self.local_rank == 0:
                log(f"[rank:{self.rank}] Available GPUs per server: {gpu_cnt}")
            ...
            self.backend = "nccl"
            log(f"[rank:{self.rank}] GPU mode is used.")
        else:
            self.backend = "gloo"
            log(f"[rank:{self.rank}] CPU mode is used.")
    ...
    def receive_tensor(self, from_rank, device):
        ...
        # log(f"[rank:{self.rank}] receive_tensor begin | dimension:{dimension} | from rank:{from_rank}")
        dist.recv(dimension, from_rank)
        # log(f"[rank:{self.rank}] receive_tensor end | dimension:{dimension} | from rank:{from_rank}")
        ...
        # log(f"[rank:{self.rank}] receive_tensor begin | shape:{shape} | from rank:{from_rank}")
        dist.recv(shape, from_rank)
        # log(f"[rank:{self.rank}] receive_tensor end | shape:{shape} | from rank:{from_rank}")
        #logging.debug(f" >>>>> recv_tensor, shaple:{shape} from rank:{from_rank}")
        shape = tuple(shape.tolist())

        ttype = torch.tensor([0], dtype=torch.long, device=device)
        # log(f"[rank:{self.rank}] receive_tensor begin | ttype:{ttype} | from rank:{from_rank}")
        dist.recv(ttype, from_rank)
        # log(f"[rank:{self.rank}] receive_tensor end | ttype:{ttype} | from rank:{from_rank}")
        ...
        # log(f"[rank:{self.rank}] receive_tensor begin | obj:{obj} | from rank:{from_rank}")
        dist.recv(obj, from_rank)
        # log(f"[rank:{self.rank}] receive_tensor end | obj:{obj} | from rank:{from_rank}")
