...
for b in ${BATCHS[@]}; do
	...
	for s in ${ZERO_STAGE[@]}; do
		...
		for g in ${GRADACCSTEP[@]}; do
			...
			sudo mv -f logs/*stdout.txt swsok-results/conf-$conf-gpunum-$i-zero-3-microbatch-$b-seq-32k-stage-$s-gradaccustep-$g-$(date '+%Y-%m-%d').txt
			sudo rm -rf logs/*
			rm checkpoints/* -rf
		done
	done
