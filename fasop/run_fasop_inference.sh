...
for TP in 1 2 4; do
	docker exec -w /workspace/aicomp/compiler_fx $CONTAINER_NAME sh -c "torchrun --nproc_per_node=$TP --nnodes=1 --node_rank=0 --master_addr=$MASTER_ADDR --master_port=29500  llama_2d_local4fasop_prof_inference_only.py $HUGGINGFACE_TOKEN | tee /workspace/tdpp/log2/tp_output/tp_output.out"
