...
#                  --master_addr="X.X.X.X" --master_port=29500 fx_dist_pp_training_type-C_gpu.py
#      <machine #1>
...
class FXRun3:
    ...
    # TODO: EXPERIMENTAL
    def run_core_backward2(self, mb_idx, node, grads):

        k1, k2 = self.fwd_cache2[mb_idx].pop(node.name)
        # k1: loss or forward outputs
        # k2: forward inputs

        if node.name == 'output':
            #gradients = torch.autograd.grad(outputs=k1, inputs=k2, retain_graph=True, allow_unused=True, materialize_grads=True,)
            gradients = torch.autograd.grad(outputs=k1, inputs=k2, retain_graph=True, allow_unused=True)
        else:
            #gradients = torch.autograd.grad(outputs=k1, inputs=k2, grad_outputs=grads, retain_graph=True, allow_unused=True, materialize_grads=True,)
            if isinstance(k1, (tuple, list)) and isinstance(k1[0], torch.Tensor) and k1[0].shape != grads[0].shape:
                print(f"<< k1[0].shape:{k1[0].shape}, grads[0].shape: {grads[0].shape}")
                if isinstance(k1, tuple):
                    k1 = list(k1)
                    k1[0] = k1[0].view(-1, grads[0].size(-1))

                    k1 = k1[0]
                    grads = grads[0]

            gradients = torch.autograd.grad(outputs=k1, inputs=k2, grad_outputs=grads, retain_graph=True, allow_unused=True)

        #forward_input_gradient = []
        #for i in range(len(k2)):
        #    forward_input_gradient[i].grad = gradients[i]
        #return forward_input_gradient

        for i in range(len(k2)):
            k2[i].grad = gradients[i]
        return k2
...
optimizer1 = Adam(fx_run3.mod.parameters(), lr=3e-5)
...
if fx_run3.rank == 0:
    #
    # move sample_input to first process
    #
...
#
# move sample_output to last process
#
if fx_run3.rank == 0:
    target_node_name = "labels"
    mbatches = torch.chunk(sample_output, fx_run3.mbsize)
...
if fx_run3.rank == fx_run3.world_size - 1:
    ...
    print(f"received <==== env2[{fx_run3.mbsize-1}][{target_node_name}]: {fx_run3.env2[fx_run3.mbsize-1][target_node_name]}")
...
for i in range(100):
    ...
    fx_run3.fx_forward4(sample_input, sample_output)
    if fx_run3.rank == fx_run3.world_size - 1:
        fx_run3.run_loss()
        loss1 = fx_run3.loss
    else:
    ...
    if sim_split.rank == sim_split.world_size - 1:
        print(f'Step {i}, Loss1:{sum(loss1) / fx_run3.mbsize}')
