                ...
                if self.optimus.tpl.stage > src_stage and self.optimus.tpl.stage <= needed_by_stage:
                    if node_name in self.optimus.run_info.getitem_dic:
                        ...
                        if self.optimus.run_info.env_recv_mark[mb_idx][submod_name] is None:
                            # log(f"[rank:{self.optimus.tpl.rank}] receive_data begin | mb_idx:{mb_idx} | from rank:{pre_split_rank}")
                            ...
                            # log(f"[rank:{self.optimus.tpl.rank}] receive_data end | mb_idx:{mb_idx} | from rank:{pre_split_rank}")
                ...
                if self.optimus.tpl.stage >= src_stage and self.optimus.tpl.stage < needed_by_stage:
                    if node_name in self.optimus.run_info.getitem_dic:
                        ...
                        if self.optimus.run_info.env_send_mark[mb_idx][submod_name] is None:
                            ...
                            # log(f"[rank:{self.optimus.tpl.rank}] send_data begin | mb_idx:{mb_idx} | to rank:{next_split_rank}")
                            self.optimus.comm.send_data(obj, next_split_rank, self.optimus.run_info.device)
                            # log(f"[rank:{self.optimus.tpl.rank}] send_data end | mb_idx:{mb_idx} | to rank:{next_split_rank}")
