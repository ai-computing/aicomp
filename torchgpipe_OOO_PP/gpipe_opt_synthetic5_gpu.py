...
#
# Large, synthetic model on torchgpipe (GPU & microbatch version)
#
...
class Hooking():
    ...
    def fwd_h(self, module, input, output):
        ...
        #print(f'forward hook called:{self.n_fwd}, LEN:{len(self.fwd_inputs))}')
    ...
    def bwd_h(self, module, input, output):
        ...
        #print(f'backward hook called:{self.n_bwd}, LEN:{len(self.grad_outputs))}')
    ...
    def compute_weight_grad(self):
        ...
        with torch.cuda.device(self.device):
            ...
            #print(f'## comp. w.g, grad_outputs: {len(self.grad_outputs)}, fwd_inputs: {len(self.fwd_inputs)}')
...
def compute_weight_grad_all(n_microbatches, n_parts):
    ...
    #            cnt = cnt + 1
    #print(f'compute_weight_grad_all: {cnt}')
...
model = GPipe(model, balance=[1,1,1,1], devices=[5,4,3,2], chunks=8, checkpoint='never')
#model = GPipe(model, balance=[1,1,1,1,1,1,1,1], devices=[7,6,5,4,3,2,1,0], chunks=8, checkpoint='never')
