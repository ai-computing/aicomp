...
#                  --master_addr="X.X.X.X" --master_port=29500 fx_dist_pp_training_type-A_remove-fwdcache.py
#      <machine #1>
    ...
    def fx_micro_backward(self, mb_idx):
        ...
        while node != from_._prev:    
            ...
            if node.op == 'call_module' or node.op == 'call_method' or \
                ...
                #    flat_args = []
                elif node.op == 'output' and use_wrapper == False:
                    output1 = self.env2[mb_idx][str(node.args[0])]
                    target1 = self.env2[mb_idx]["labels"]
                    #kwargs = {}
                    #flat_args = []
                ...
                #kwargs = dict(kwargs)
                kwargs = dict() 
                k1 = result if isinstance(result, tuple) else (result,)
                k2 = flat_args
