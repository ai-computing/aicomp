...
#                 --master_addr=<IP_of_rank_0> --master_port=29500 pp_train_llama6.py <llama_access_token>
#
...
#def get_info(optimus):
#    param_info = []
#    for name, param in optimus.run_info.submod.named_parameters():
#        param_info.append(f"Name: {name}, Shape: {param.shape}")
#
#    file_name = f"tensor_info_rank{optimus.get_rank()}.txt"
#    with open(file_name, "w") as f:
#        f.write(f"Rank {optimus.get_rank()} Parameter Info:\n")
#        f.write("\n".join(param_info))
#    print(f" [{optimus.get_rank()}] get_info ..........")
...
#optimus_p = Optimus_p(model, micro_batch_size, use_gpu=True, pp_size=2, tp_size=8, activation_ckpt=True, force_free_mem=True, display_mem=True, swap_opt_in_fwdbwd=True, swap_model_in_optstep=False, ir_analyze=IR_Anal.SEQUENTIAL)
#optimus_p = Optimus_p(model, micro_batch_size, use_gpu=True, tp_size=8, activation_ckpt=True, force_free_mem=True, display_mem=True, swap_opt_in_fwdbwd=True, swap_model_in_optstep=False, ir_analyze=IR_Anal.SEQUENTIAL)
optimus_p = Optimus_p(model, micro_batch_size, use_gpu=True, tp_size=2, dp_size=2, activation_ckpt=True, force_free_mem=True, display_mem=True, swap_opt_in_fwdbwd=True, swap_model_in_optstep=False, ir_analyze=IR_Anal.SEQUENTIAL)

#optimus_p = Optimus_p(model, micro_batch_size, use_gpu=True, pp_size=4, tp_size=4, activation_ckpt=True, force_free_mem=True, display_mem=True, swap_opt_in_fwdbwd=True, swap_model_in_optstep=True, ir_analyze=IR_Anal.SEQUENTIAL)
