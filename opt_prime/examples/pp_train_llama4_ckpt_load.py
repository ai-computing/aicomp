...
#                 --master_addr=<IP_of_rank_0> --master_port=29500 pp_train_llama4_ckpt_load.py <llama_access_token>
#
...
from transformers import LlamaConfig # TODO
...
if access_token is None:
    raise ValueError("LLAMA_ACCESS_TOKEN environment variable is not set."
                    "       [Usage:] torchrun --nproc_per_node=<#_of_GPUs_per_node> --nnodes=<#_of_nodes> --node_rank=<current_node_rank> --master_addr=<IP_of_rank_0> --master_port=29500 pp_train_llama4_ckpt_load.py <llama_access_token>")
...
config = LlamaConfig.from_pretrained("meta-llama/Llama-3.2-1B", token=access_token, use_cache=False)
...
optimus_p = Optimus_p(model, micro_batch_size, use_gpu=True, activation_ckpt=True, force_free_mem=True, display_mem=True, swap_opt_in_fwdbwd=True, swap_model_in_optstep=True, ir_analyze=IR_Anal.SEQUENTIAL, ckpt_dir_postfix="llama")
