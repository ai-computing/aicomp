...
#                  --master_addr="X.X.X.X" --master_port=29500 fx_dist_pp_training_type-A_gpu_mopt1.py
#      <machine #1>
...
class FXRun2:
    ...
    def has_child(self, mod):
        num_children = 0
        #print(f" has_child --- mod:{mod}, list(mod.children()):{list(mod.children())}")
        if mod is not None:
            num_children = len(list(mod.children()))
            #print(f"children = {list(mod.children())}")
        return num_children
...
if use_gpu == True:
    ...
    fx_run2.mod.to(fx_run2.device) # move memory cleaned modules to GPU
