...
from .IR import IR, IR_Anal
from .schedule import ScheduleGPipe 
from .schedule import Schedule1F1B 
...
from .utils import ts, log, log_special_nodes
...
class Optimus_p:
    ...
    def prepare_dataloader(self, datasets, batch_size):
        if self.tpl.dp_size > 1:
            ...
            local_batch_size = batch_size // self.tpl.dp_size # each rank handles local_batch_size samples
            return DataLoader(datasets, 
                              batch_size=local_batch_size, 
                              num_workers=4, 
                              sampler=DistributedSampler(datasets, shuffle=True, num_replicas=self.tpl.dp_size, rank=dp_rank))
        else:
