...
model = model.from_pretrained("gpt2", num_labels=3)
...
def train():
    ...
    for i, batch in enumerate(dataloader):
        ...
        if optimus_p.is_first_stage():
            tokens =  tokenizer(batch["text"], padding=True, truncation=True, max_length=1024,return_tensors="pt")
            data, labels = tokens.input_ids, batch["label"]
