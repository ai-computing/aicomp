...
#
#
#  This is a test program for running Varuna with pipeline-parallel and data-parallel training.
#
#
#  Sample Usage for PP and DP:
#
#      # python3 -m varuna.run_varuna --machine_list ip_list --gpus_per_node 4 --batch_size 64 --nstages 4 --chunk_size 2 varuna_pp_training.py
#
...
num_cutpoints = 3 # for pp+dp processes (4+4)
...
def get_batch_fn2(size, device=None):
    ...
    # for the first stage of PP+DP
    if args.rank == 0 or args.rank == 4:
        inputs = {"inputs": input, "labels":None}
        if device is not None:
            inputs["inputs"] = inputs["inputs"].to(device)

    # for the last stage of PP+DP
    elif args.rank == 3 or args.rank == args.world_size -1:
