                ...
                if self.run_info.rank > src_rank and self.run_info.rank <= needed_by_rank:
                    ...
                    if node_name in self.run_info.getitem_dic:
                        ...
                        if self.run_info.env_recv_mark[mb_idx][submod_name] is None:
                            #print(f"*** FMF--[rank:{self.run_info.rank}] mb_idx:{mb_idx}, submod_name:{submod_name}, before receive_data(from:{pre_split_rank})") # TODO: TO DELETE
                            self.run_info.env[mb_idx][submod_name] = self.comm.receive_data(pre_split_rank, self.run_info.device)
                            #print(f"*** FMF--[rank:{self.run_info.rank}] mb_idx:{mb_idx}, submod_name:{submod_name}, after receive_data(from:{pre_split_rank})") # TODO: TO DELETE
                        ...
                        if self.run_info.env_recv_mark[mb_idx][node_name] is None:
                            #print(f"*** FMF-else[rank:{self.run_info.rank}] mb_idx:{mb_idx}, node_name:{node_name}, before receive_data(from:{pre_split_rank})") # TODO: TO DELETE
                            self.run_info.env[mb_idx][node_name] = self.comm.receive_data(pre_split_rank, self.run_info.device)
                            #print(f"*** FMF-else[rank:{self.run_info.rank}] mb_idx:{mb_idx}, node_name:{node_name}, after receive_data(from:{pre_split_rank})") # TODO: TO DELETE
...
class Schedule1F1B:
    ...
    def run(self, data, labels):
        ...
        for i in range(num_warmup_microbatches):
            ...
            result = self.post_fx_micro_forward_core(i)
            next(result)
            #print(f" [stage:{self.run_info.stage}] fx_micro_forward({i}) ..") # TODO: TO DELETE
        ...
        for i in range(remaining): # steady
            ...
            next(result)
            #print(f" [[stage:{self.run_info.stage}]] fx_micro_forward({forward_i}) ..") # TODO: TO DELETE

            if self.run_info.rank == self.run_info.world_size - 1:
                self.run_loss(backward_i)
                #print(f" [[stage:{self.run_info.stage}]] run_loss({backward_i}) ..") # TODO: TO DELETE
            ...
            #print(f" [[stage:{self.run_info.stage}]] fx_micro_backward({backward_i}) ..") # TODO: TO DELETE
            ...
            for node_name, range_ in self.run_info.special_nodes.items():
                ...
                if self.run_info.rank >= src_rank and self.run_info.rank < needed_by_rank:
                    if node_name in self.run_info.getitem_dic:
                        ...
                        if self.run_info.env_send_mark[mb_idx][submod_name] is None:
                            obj = self.run_info.env[mb_idx][submod_name]
                            #print(f"*** FMF-[rank:{self.run_info.rank}] mb_idx:{mb_idx}, submod_name:{submod_name}, before send_data(to:{next_split_rank})") # TODO: TO DELETE
                            self.comm.send_data(obj, next_split_rank, self.run_info.device)
                            #print(f"*** FMF-[rank:{self.run_info.rank}] mb_idx:{mb_idx}, submod_name:{submod_name}, after send_data(to:{next_split_rank})") # TODO: TO DELETE
                            self.run_info.env_send_mark[mb_idx][submod_name] = 1
                    else:
                        if self.run_info.env_send_mark[mb_idx][node_name] is None:
                            obj = self.run_info.env[mb_idx][node_name]
                            #print(f"*** FMFelse[rank:{self.run_info.rank}] mb_idx:{mb_idx}, node_name:{node_name}, before send_data(to:{next_split_rank})") # TODO: TO DELETE
                            self.comm.send_data(obj, next_split_rank, self.run_info.device)
                            #print(f"*** FMFelse[rank:{self.run_info.rank}] mb_idx:{mb_idx}, node_name:{node_name}, after send_data(to:{next_split_rank})") # TODO: TO DELETE
                            self.run_info.env_send_mark[mb_idx][node_name] = 1
    ...
    def pre_fx_micro_backward_core(self, mb_idx):
        ...
        if self.run_info.rank < self.run_info.world_size - 1:
            ...
            node_name = self.get_next_node_name(self.run_info.rank)
            if self.run_info.env_grad_recv_mark[mb_idx][node_name] is None:
                #print(f"*** FMbackward,[rank:{self.run_info.rank}] mb_idx:{mb_idx}, node_name:{node_name}, before receive_data(from:{pre_split_rank})") # TODO: TO DELETE
                self.run_info.grads[mb_idx][node_name] = self.comm.receive_data(pre_split_rank, self.run_info.device)
                #print(f"*** FMbackward,[rank:{self.run_info.rank}] mb_idx:{mb_idx}, node_name:{node_name}, after receive_data(from:{pre_split_rank})") # TODO: TO DELETE
                grads = self.run_info.grads[mb_idx][node_name]
                self.run_info.env_grad_recv_mark[mb_idx][node_name] = 1
    ...
    def post_fx_micro_backward_core(self, mb_idx):
        if self.run_info.rank > 0:
            ...
            if self.run_info.env_grad_send_mark[mb_idx][node_name] is None:
                obj = self.run_info.grads[mb_idx][node_name]
                #print(f"***** FMBackward,[rank:{self.run_info.rank}] mb_idx:{mb_idx}, node_name:{node_name}, before send_data(to:{next_split_rank})") # TODO: TO DELETE
                self.comm.send_data(obj, next_split_rank, self.run_info.device)
                #print(f"***** FMBackward,[rank:{self.run_info.rank}] mb_idx:{mb_idx}, node_name:{node_name}, after send_data(to:{next_split_rank})") # TODO: TO DELETE
                self.run_info.env_grad_send_mark[mb_idx][node_name] = 1
