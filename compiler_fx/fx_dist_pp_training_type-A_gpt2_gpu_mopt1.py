...
#                  --master_addr="X.X.X.X" --master_port=29500 fx_dist_pp_training_type-A_gpt2_gpu_mopt1.py
#      <machine #1>
...
#def get_total_params2(module: torch.nn.Module):
#    total_params = 0
#    for param_name, param in module.named_parameters():
#        total_params += param.numel()
#        print(f" ### param_name:{param_name}, numel:{param.numel()}")
#    return total_params
...
class FXRun2:
    ...
    def delete_param(self, mod, name):
        #print(f" del param : {mod}, name:{name}")
    ...
    def delete_module(self, root, module_node):
        ...
        if len(module_name) == len(module_target_name) and not self.partially_matched(module_target_name):
            ...
            if get_attr_flag == True or self.has_child(attr_itr) == 0:
                ##print(f">>> BEFORE  rank:{self.rank}, delete module:{module_name},target:{module_target_name} ## atom:{atom},op:{module_node.op}")

                self.delete_param(attr_itr, atom)
                delattr(parent_, atom)
                del attr_itr

                #print(f">>>  rank:{self.rank}, delete module:{module_name} ## atom:{atom},op:{module_node.op}")
                #print(f">>> rank:{self.rank}, delete module:target:{module_target_name} ## atom:{atom},op:{module_node.op}")
                ##print(f">>> AFTER rank:{self.rank}, delete module:{module_name},target:{module_target_name} ## atom:{atom},op:{module_node.op}")

                self.reference.append(module_target_name)
                #self.reference.append(str('.'.join(target_atoms)))
    ...
    def clean_unused_modules(self):
        ...
        for rank in reversed(range(self.world_size)):
            ...
            if rank != self.rank:
                from_, to_ = self.get_range(rank, self.graph)
...
#if int(os.environ['RANK'])== 0:
#    print(f"RANK:{os.environ['RANK']}, Total parameters in model: {get_total_params2(model)}")
#sys.exit(0)
...
if use_gpu == True:
    ...
    fx_run2.mod.to(fx_run2.device)
    print_memory_usage("After moved into GPU", print_mem)
...
print(f"RANK:{fx_run2.rank}, Total parameters in model: {get_total_params(fx_run2.mod)}")

print_memory_usage("After get_total_params", print_mem)
